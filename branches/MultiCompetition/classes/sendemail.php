<?php
//@ validate inclusion
if(!defined('VALID_ACCESS_SENDEMAIL_')) exit(basename(__FILE__) . ' -> direct access is not allowed.');
...
class cPHPezMail
{
	...
	function SetEncodingBit($str_Encoding)
	{
		$this->sEncoding = $str_Encoding;
	}
	...
	function Send()
	{
		...
		$message = $this->_MakeMessage();
		$header = $this->_MakeHeader();
		$bResponse["MailTo"] = $sTo;
		$bResponse["MailSubject"] = $subject;
		$bResponse["MailContent"] = $message;
		$bResponse["MailHeader"] = $header;
		
		$bResponse["MailResponse"] = mail($sTo,$subject , $message, $header);
