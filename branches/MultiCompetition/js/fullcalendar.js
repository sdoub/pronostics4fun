...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		var _element = this,
			...
			elementOuterWidth,
			content = $("<div class='fc-content " + tm + "-widget-content' style='position:relative' height='450px'/>").prependTo(_element),
			suggestedViewHeight,
			resizeUID = 0,
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					s += "<td class='fc-" +
						dayIDs[d.getDay()] + ' ' + // needs to be first
						tm + '-widget-content fc-day' + (i*colCnt+j) +
						(j==dit ? ' fc-leftmost' : '') +
			...
			if (rowCnt < prevRowCnt) {
				...
				for (i=prevRowCnt; i<rowCnt; i++) {
					...
					for (j=0; j<colCnt; j++) {
						s += "<td class='fc-" +
							dayIDs[d.getDay()] + ' ' + // needs to be first
							tm + '-widget-content fc-new fc-day' + (i*colCnt+j) +
							(j==dit ? ' fc-leftmost' : '') + "'>" +
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		html +=
			"<div " + (event.matchKey ? " match-key='" + event.matchKey + "'" : '') + " class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" +
				"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
					...
					"<span class='fc-event-time'>" +
					htmlEscape(formatDates(event.start, event.end, "hh:mm", options)) +
					"</span>" +
				"</a>" +
...
function Agenda(element, options, methods) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			if (options.allDaySlot) {
				s += "<tr class='fc-all-day'>" +
						...
						"<td colspan='" + colCnt + "' class='" + tm + "-widget-content'>" +
							"<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td>" +
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			html +=
				"<div " + (event.matchKey ? " match-key='" + event.matchKey + "'" : '') + " class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + top + "px;left:" + left + "px'>" +
					"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
