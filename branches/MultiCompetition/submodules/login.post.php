...
if($_isAuthenticated)
{
  ...
  echo "{'status':true,'message':'Vous êtes déjà connecté','url':'index.php'}";
}
else
{
  //@ session not active
  ...
  else
  {
    ...
    if ($pf) {
      ...
      if ($is_reset) {
        //@ success
        $arr["status"] = true;
        $arr["message"] = "Un email vous a été envoyé avec la procédure à suivre.";
      }
      else {
        $arr["status"] = false;
      ...
      if($is_auth)
      {
        //@ success
        $arr["status"] = true;
        $arr["message"] = "Connexion autorisée";
        $arr["url"] = "index.php";
      }
      else
      {
        //@ failed
        $arr["status"] = false;
        $arr["message"] = "Echec de connexion : veuillez vérifier vos informations de connexion.";
      }
    }
  }
