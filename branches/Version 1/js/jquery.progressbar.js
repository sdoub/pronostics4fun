	...
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				...
				showTooltip		: true,											// show tooltip? - default : true
				textFormat		: 'percentage',									// Or otherwise, set to 'fraction'
				tooltipFormat	: 'percentage',									// Or otherwise, set to 'fraction'
				...
				return this.each(function(child) {
					...
					function getTooltip(config) {
						if (config.showTooltip) {
							if (config.tooltipFormat == 'percentage') {
								return " " + Math.round(config.running_value) + "%";
							} else if (config.tooltipFormat == 'fraction') {
								return " " + config.running_value + '/' + config.max;
							} else  {
								return " " + config.running_value + config.tooltipFormat;
							}
						}
					}
					...
					if (argvalue != null && this.bar != null && this.config != null) {
						...
						$bar.attr('title', getTooltip(config));
						$bar.attr('alt', getTooltip(config));
					...
					var t = setInterval(function() {
						...
						if (config.running_value == config.max && config.complete != null && typeof(config.complete) == 'function')
