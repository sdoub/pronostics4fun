...
if($_SERVER['REQUEST_METHOD']=='GET')
{
  ...
  $htmlContent = "<div style='height:" . $divHeight . "px;'> <img src='" .ROOT_SITE ."/images/warning.png' style='height:50px;width:50px;float:left;padding-right:5px;'/> <div style='color:red;font-size:10px;'>Malheureusement le site est actuellement indisponible, n�anmoins, afin que vous ne perdiez pas de pr�cieux points, vous pouvez utiliser le formulaire ci-dessous pour donner vos pronostics sur les matchs � venir!<br/>Bon pronostics et d�sol� pour le d�sagr�ment occasion�.</div><div style='text-align: right;color:#000;font-weight:normal;padding-top:10px;font-size:10px;'>L'administrateur de Pronostics4Fun.</div><hr/><form id='frmForecast'>
<div><label>Veuillez indiquer votre adresse email utilis� sur Pronostics4Fun</label><input
  ...
  while (list ($key, $value) = each ($matches)) {
    ...
    if (!($scheduleMonth==$tempScheduleMonth && $scheduleDay==$tempScheduleDay))
    {
      ...
      $htmlContent.= '<tr class="day"
      	    style="">
      	  <td colspan="6">Journée&nbsp;' . $value["DayKey"] . '&nbsp;->&nbsp;' . $scheduleFormattedDate . '</td>';
...
<div id="footerForecast" ><input type="submit"
  ...
  */
  ...
  $arr["queries"]=$queries;
    ...
    $arr["status"] = false;
    $arr["queries"]=$queries;
    $arr["message"] = "Veuillez vérifier votre adresse email, elle est incorrecte!";
