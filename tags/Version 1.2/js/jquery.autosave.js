...
;(function($) {
	...
	$.fn.autosave.go = function () {
		...
		function saveCookie (i, j, content)
		{
			...
			exipryCookieDate.setDate(exipryCookieDate.getDate() + $.fn.autosave.options.cookieExpiryLength );

			$.cookies.set('autosave_'+u+i+'_'+j,content, { expiresAt: exipryCookieDate});
		}
	
		function removeBiggerCookies (i)
		{
			var j = 1;
			while ($.cookies.get('autosave_'+u+i+'_'+j) !== null && j < 20)
			{
				$.cookies.del('autosave_'+u+i+'_'+j);
			}
		}
		...
		$.cookies.set('autosave_'+u+'_check', cookiecheck);
		...
		$.cookies.set('autosave_'+u+'_radio', cookieradio);
	...
	$.fn.autosave.restore = function () {
		...
		for (i in m.text)
		{
			...
			while ($.cookies.get('autosave_'+u+i+'_'+j) !== null && j < 20)
			{
				restored += $.cookies.get('autosave_'+u+i+'_'+j);
				j += 1;
			}
			$(m.text[i]).val(restored);
		}
	
		var cookiecheck = $.cookies.get('autosave_'+u+'_check').split(',');
		if (cookiecheck !== null)
		{
		...
		var cookieradio = $.cookies.get('autosave_'+u+'_radio').split(',');
		if (cookieradio !== null)
		{
	...
	$.fn.autosave.removeAllCookies = function () {
		...
		$.cookies.del('autosave_'+u+'_check');
		$.cookies.del('autosave_'+u+'_radio');
	};
