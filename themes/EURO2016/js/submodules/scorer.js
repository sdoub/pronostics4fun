...
var wrapperId 	=	'#wrapper';		// main container
	...
	var postFile	=	'submodule.post.php?SubModule=11';	// post handler
	...
	function getScorerDetail (teamPlayerKey,teamKey) { 

		_teamPlayerKey = teamPlayerKey;
		_teamKey = teamKey;
	// hide first
	$(waitId).hide(); $(wrapperId).hide();
	...
	$(waitId).html(waitNote).fadeIn('slow',function(){
		...
		$.ajax({
			  url: postFile + "&teamPlayerKey="+teamPlayerKey+ "&teamKey="+teamKey,
			  dataType: 'json',
			  data: "",
