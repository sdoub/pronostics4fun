...
class Championship {
    ...
    public function create($list = array()) {
        ...
        for ($round = 0; $round < $n-1; $round++) {
            if ($this->rot_out != -1) # If not ghost player.
                array_push($this->tour, array("Round"=>$round+1,"Home"=>$list[$this->fixed], "Away"=>$list[$this->rot_out]));
            for ($i = 0; $i <= count($this->upper)-1; $i++) { # -1 because of 0th element.
                if ($this->upper[$i] != -1 && $this->lower[$i] != -1) # If not ghost player.
                    array_push($this->tour, array("Round"=>$round+1,"Home"=>$list[$this->upper[$i]], "Away"=>$list[$this->lower[$i]]));
            }
            $this->rotate();
        }
    ...
    public function ToArray() {
      return array("Competitors"=>$this->tot_competitors,"Games"=>$this->tot_games,"Rounds"=>$this->tour);
    }
    // Rotate arrays
